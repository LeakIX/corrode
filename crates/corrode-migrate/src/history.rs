...
//!
//! This module manages the `corrode_migrations` table that tracks which migrations
...
CREATE TABLE IF NOT EXISTS corrode_migrations (
...
impl MigrationHistory {
    ...
    pub async fn record_applied(&self, app: &str, name: &str) -> Result<()> {
        sqlx::query("INSERT INTO corrode_migrations (app, name) VALUES (?, ?)")
            .bind(app)
            .bind(name)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn record_unapplied(&self, app: &str, name: &str) -> Result<()> {
        let result = sqlx::query("DELETE FROM corrode_migrations WHERE app = ? AND name = ?")
            .bind(app)
            .bind(name)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn is_applied(&self, app: &str, name: &str) -> Result<bool> {
        let row: Option<(i64,)> =
            sqlx::query_as("SELECT 1 FROM corrode_migrations WHERE app = ? AND name = ?")
                .bind(app)
                .bind(name)
    ...
    pub async fn get_applied(&self) -> Result<Vec<AppliedMigration>> {
        let rows: Vec<(i64, String, String, String)> =
            sqlx::query_as("SELECT id, app, name, applied_at FROM corrode_migrations ORDER BY id")
                .fetch_all(&self.pool)
                .await?;
    ...
    pub async fn get_applied_for_app(&self, app: &str) -> Result<Vec<AppliedMigration>> {
        let rows: Vec<(i64, String, String, String)> = sqlx::query_as(
            "SELECT id, app, name, applied_at FROM corrode_migrations WHERE app = ? ORDER BY id",
        )
        .bind(app)
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn get_last_applied(&self, app: &str) -> Result<Option<AppliedMigration>> {
        let row: Option<(i64, String, String, String)> = sqlx::query_as(
            "SELECT id, app, name, applied_at FROM corrode_migrations WHERE app = ? ORDER BY id DESC LIMIT 1",
        )
        .bind(app)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn count_applied(&self) -> Result<i64> {
        let row: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM corrode_migrations")
            .fetch_one(&self.pool)
            .await?;
        Ok(row.0)
    }
    ...
    pub async fn count_applied_for_app(&self, app: &str) -> Result<i64> {
        let row: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM corrode_migrations WHERE app = ?")
            .bind(app)
            .fetch_one(&self.pool)
            .await?;
        Ok(row.0)
    ...
    pub async fn get_applied_set(&self) -> Result<std::collections::HashSet<(String, String)>> {
        let rows: Vec<(String, String)> =
            sqlx::query_as("SELECT app, name FROM corrode_migrations")
                .fetch_all(&self.pool)
                .await?;
