//! Blog Admin Example
//!
//! This example demonstrates the corrode-admin interface with a blog application.
...
use corrode_admin::{AdminSite, Fieldset, ModelAdmin};
use corrode_router::{Method, Request, Response, Router};
...
impl corrode_orm::Model for Post {
...
impl corrode_orm::Model for Comment {
...
impl corrode_orm::Model for Tag {
    type Table = TagTable;
    type PrimaryKey = i64;
...
fn init_sample_data() -> DataStore {
    DataStore {
        posts: vec![
            Post {
                id: 1,
                title: "Welcome to Corrode Blog".to_string(),
                slug: "welcome-to-corrode-blog".to_string(),
...
async fn handle_request(
    ...
    // Convert hyper request to corrode_router Request
    let method = Method::parse(req.method().as_str()).unwrap_or(Method::Get);
    let uri = req.uri();
    let path = uri.path().to_string();

    let mut corrode_req = Request::new(method, &path);

    // Parse query string
    if let Some(query) = uri.query() {
        corrode_req.query = Request::parse_query_string(query);
    }

    // Copy headers
    for (key, value) in req.headers() {
        if let Ok(v) = value.to_str() {
            corrode_req.headers.insert(key.to_string(), v.to_string());
        }
    }
    ...
    let body_bytes = req
        ...
        .map(|b| b.to_bytes())
        .unwrap_or_default();
    corrode_req.body = body_bytes.to_vec();

    // Handle request
    let corrode_res = router.handle(corrode_req).await;

    // Convert corrode_router Response to hyper Response
    let mut builder = HyperResponse::builder().status(
        StatusCode::from_u16(corrode_res.status).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
    );

    for (key, value) in &corrode_res.headers {
        builder = builder.header(key.as_str(), value.as_str());
    }

    let response = builder
        .body(Full::new(Bytes::from(corrode_res.body)))
        .unwrap();
